...
open class ObjectUIRelations: NSObject {
    ...
    func connectView(_ view: UIView) -> Bool {
        ...
        if let view = view as? UISwitch {
            ...
            Notification.subscribe(observer: self, selector: #selector(ObjectUIRelations.onTextField(_:)), name: UITextField.textDidChangeNotification, object: view)
            connected = true
    ...
    func disconnectView(_ view: UIView) {
        if let view = view as? UISwitch {
            ...
            Notification.unsubscribe(observer: self, name: UITextField.textDidChangeNotification, object: view)
        } else if let view = view as? UISegmentedControl {
